...
def train(
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        train_res = run_epoch(
            ...
            is_train=True, model_type=model_type,
        ...
        val_res = run_epoch(
            ...
            is_train=False, model_type=model_type,
        ...
        if verbose:
            ...
            if 'crf_metrics' in val_res and val_res['crf_metrics']:
                ...
                crf_str = (f" | cells={cm['n_cells_mean']:.1f}"
                           f" splits={cm['splits_per_ex']:.1f}"
                           f" spec={cm['specialization']:.3f}")
            print(f"[{run_name}] ep {epoch:3d}/{n_epochs} "
...
def count_gpt_params(vocab_size, d_model, n_layers, tie_weights=True, activation='swiglu'):
    """
...
def make_matched_transformer(
    ...
    for d in d_values:
        for L in l_values:
            ...
            p = count_gpt_params(vocab_size, d, L, tie_weights=True, activation='swiglu')
    ...
    print(f"    [Transformer match] d={d} L={L} h={n_heads} "
...
if __name__ == '__main__':
    ...
    model    = CRFLanguageModel(
        vocab_size=tok_size, d_model=64, d_hidden=32,
        n_init_cells=16, max_cells=64, n_crf_steps=4,
        k_neighbors=3, cfg=cfg,
    )
