...
from crf_reasoning.ablations import (
    ABLATION_CONFIGS,
...
from crf_reasoning.metrics import (
    ...
    measure_memory_mb,
    validate_bounded_communication,
    validate_adaptive_computation,
    validate_energy_conservation,
    fit_scaling_law,
    compute_graph_diameter_from_states,
)
...
def collect_inference_profile(
    model, model_type: str, va_dl, n_batches: int = 5
) -> Dict:
...
def exp_main_comparison(ec: Dict, results: Dict) -> None:
    ...
    for ds_name in datasets:
        ...
        tr_ds, va_ds = get_datasets(
            ds_name, ec["seq_len"], ec["n_train"], ec["n_val"], tok
        )
        ...
        hist_t = run_one(
            tr_model, "transformer", tr_dl, va_dl, ec, f"transformer_{ds_name}"
        )
...
def exp_ablations(ec: Dict, results: Dict) -> None:
    ...
    for ds_name in abl_datasets:
        ...
        for abl_name in ABLATION_CONFIGS:
            ...
            hist = run_one(model, "crf", tr_dl, va_dl, ec, f"abl_{abl_name}_{ds_name}")
...
def exp_scaling(ec: Dict, results: Dict) -> None:
    ...
    vocab = tok.vocab_size
    tr_ds, va_ds = get_datasets(
        "synthetic", ec["seq_len"], ec["n_train"], ec["n_val"], tok
    )
    tr_dl = make_dataloader(tr_ds, ec["batch_size"])
    va_dl = make_dataloader(va_ds, ec["batch_size"], shuffle=False)
    ...
    for n_cells in ec["scale_cells"]:
        ...
        flops = estimate_crf_flops(
            n_cells, ec["d_model"], ec["d_hidden"], ec["k_neighbors"], ec["n_crf_steps"]
        )
    ...
    for s in ec["scale_steps"]:
        ...
        flops = estimate_crf_flops(
            ec["n_init_cells"], ec["d_model"], ec["d_hidden"], ec["k_neighbors"], s
        )
    ...
    for d in [32, 48, 64, 96, 128, 160, 192]:
        ...
        flops = estimate_crf_flops(
            ec["n_init_cells"], d, max(16, d // 2), ec["k_neighbors"], ec["n_crf_steps"]
        )
...
def exp_theory_validation(ec: Dict, results: Dict) -> None:
    ...
    print(
        f"  Theorem 1 (bounded comm): holds={t1['theorem_holds']} "
    ...
    print(
        f"  Theorem 2 (adaptive compute): adaptive={t2['adaptive']} "
    ...
    print(
        f"  Lemma (energy conservation): {t3['energy_lemma']}, "
...
def exp_profiling(ec: Dict, results: Dict) -> None:
    ...
    for name, model, mtype in [
        ...
        prof["n_params"] = sum(p.numel() for p in model.parameters() if p.requires_grad)
        ...
        print(
            f"    infer={prof['latency_ms_mean']:.1f}ms  "
...
def main():
    ...
    parser.add_argument(
        "--fast", action="store_true", help="Use tiny model/data for quick testing"
    )
    parser.add_argument(
        "--exp",
        type=str,
    ...
    print(
        f"Config: d={ec['d_model']} N={ec['n_init_cells']} S={ec['n_crf_steps']} "
