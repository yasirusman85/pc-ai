...
def table_ablation_metrics(results: Dict) -> str:
    ...
    lines.append(f"{'─'*80}")
    hdr = (f"  {'Ablation':<22} {'Avg N':>7} {'Splits/ex':>10} "
    ...
    for name, data in syn.items():
        ...
        if not cm_list:
            lines.append(f"  {name:<22} {'—':>7} {'—':>10} {'—':>10} {'—':>8} {'—':>10}")
            continue
...
def table_scaling(results: Dict) -> str:
    ...
    for k, v in sc.get('cells', {}).items():
        lines.append(f"  {k:>8} {fmt(v.get('best_val_ppl')):>10} "
...
def table_theory(results: Dict) -> str:
    ...
    lines.append(f"    Violations:      {t1.get('violations', '?')} / {t1.get('n_checked', '?')}")
...
def table_profiling(results: Dict) -> str:
    ...
    lines.append(f"{'─'*70}")
    hdr = (f"  {'Model':<14} {'Params':>10} {'Infer ms':>10} "
...
def make_figures(results: Dict) -> None:
    ...
    if abl:
        ...
        fig, axes = plt.subplots(1, len(datasets_abl), figsize=(6*len(datasets_abl), 5))
    ...
    if sc:
        ...
        for ax, key, xlabel in [
            ...
            ax.plot([int(x) for x in xs], ppls, 'o-', color='#2196F3',
        ...
        if dm_data:
            ...
            ax3.plot([int(x) for x in xs], ppls, 's-', color='#4CAF50',
                     linewidth=2, markersize=7)
...
def main():
    ...
    if cfg:
        print(f"  Config: d={cfg.get('d_model')} N={cfg.get('n_init_cells')} "
    ...
    with open(tables_path, 'w') as f:
        ...
        for fn in [table_main_comparison, table_ablations, table_ablation_metrics,
                   table_scaling, table_theory, table_profiling]:
            f.write(fn(results) + '\n')
