...
class TransformerBlock(nn.Module):
    def __init__(self, d_model, n_heads, d_ff, dropout=0.1, activation='swiglu', use_rotary=False):
        super().__init__()
...
class GPT(nn.Module):
    def __init__(self, vocab_size, d_model=768, n_heads=12, n_layers=12,
                 d_ff=None, max_seq_len=2048, dropout=0.1, activation='swiglu',
                 use_rotary=False, tie_weights=False):
        super().__init__()
        ...
        self.pos_enc = SinusoidalPositionalEncoding(d_model, max_seq_len) if not use_rotary else None

        self.blocks = nn.ModuleList([
            TransformerBlock(d_model, n_heads, d_ff, dropout, activation, use_rotary)
    ...
    def forward(self, x, targets=None):
        ...
        if self.use_rotary:
            if self.cos is None or self.cos.size(0) < T:
                ...
                self.cos, self.sin = precompute_rotary_cache(head_dim, self.max_seq_len, x.device)
    ...
    def generate(self, x, max_new_tokens=100, temperature=1.0, top_k=None, top_p=None):
        ...
        for _ in range(max_new_tokens):
            ...
            if top_p is not None:
                ...
                logits.scatter_(1, sorted_idx, torch.where(mask, float('-inf'), logits.gather(1, sorted_idx)))
...
if __name__ == '__main__':
    vocab_size = 50257
    model = GPT(vocab_size=vocab_size, d_model=768, n_heads=12, n_layers=12,
                max_seq_len=2048, use_rotary=False)
    ...
    rotary_model = GPT(vocab_size=vocab_size, d_model=768, n_heads=12, n_layers=12,
                       max_seq_len=2048, use_rotary=True)
