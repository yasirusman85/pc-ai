...
def _make_story(seed: int, length: int = 200) -> str:
    ...
    nouns   = ['cat','dog','bird','child','king','woman','man','city','tree','sun']
    ...
    for _ in range(n):
        kind = rng.random()
        if kind < 0.5:
            s = (f"The {rng.choice(adjs)} {rng.choice(nouns)} "
            ...
            s = (f"Once upon a time a {rng.choice(adjs)} {rng.choice(nouns)} "
    ...
    def __init__(
        ...
        for _ in range(n_samples):
            if difficulty == 'easy' or (difficulty == 'mixed' and rng.random() < 0.5):
                ...
                text = (f"Q: If you have {a} apples and get {b} more "
...
class ChainOfThoughtDataset(Dataset):
    ...
    def difficulty_split(self, easy_max_steps: int = 2):
        ...
        easy = [i for i, n in enumerate(self.n_steps_per_example) if n <= easy_max_steps]
...
class ARCProxyDataset(Dataset):
    ...
    def __init__(
        ...
        for _ in range(n_samples):
            ...
            rows = [[rng.randint(0, 9) for _ in range(grid_size)]
        ...
        try:
            ...
            if name in ['humaneval', 'gsm8k', 'arc']:
                train_ds = RealDatasetFactory.create_dataset(
                    name, split='train', seq_len=seq_len, max_samples=max_train, tokenizer=tok
                )
                val_ds = RealDatasetFactory.create_dataset(
                    name, split='test', seq_len=seq_len, max_samples=max_val, tokenizer=tok
                )
