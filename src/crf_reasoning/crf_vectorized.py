...
class VectorizedCRF(nn.Module):
    ...
    def forward(
        ...
        anchor_pos  = torch.stack([
            torch.tensor([float(i % n_grid), float(i // n_grid)])
            for i in range(T)
        ], dim=0).to(device)
        n_extra = n_create - T
        extra_idxs  = torch.arange(n_extra, device=device) % T if n_extra > 0 else torch.tensor([], device=device, dtype=torch.long)
        extra_pos   = (anchor_pos[extra_idxs] + (
            torch.rand(n_extra, 2, device=device) * 0.6 - 0.3
        )) if n_extra > 0 else torch.zeros(0, 2, device=device)
        ...
        for b in range(B):
            seq_H = H[b]                                       # T×d
            anchor_s = seq_H                                    # T×d
            extra_s  = (seq_H[extra_idxs] + torch.randn(n_extra, d, device=device) * 0.02
                       ) if n_extra > 0 else torch.tensor([], device=device).reshape(0, d)
            ...
            batch_ids.append(torch.full((N_per_seq,), b, device=device, dtype=torch.long))
        ...
        for step in range(n_steps):
            ...
            if cfg.use_energy:
                pop.energies = (
                    0.90 * pop.energies + 0.10 * energy_delta * 2
                ).clamp(0.01, 5)
            else:
...
class CRFLanguageModel(nn.Module):
    ...
    def __init__(
        ...
        self.crf          = VectorizedCRF(d_model, d_hidden, n_init_cells,
                                          max_cells, k_neighbors, self.cfg)
    ...
    def forward(
        ...
        return logits, loss, all_met if collect_metrics else None
...
if __name__ == '__main__':
    ...
    for use_routing in [True, False]:
        for use_energy in [True, False]:
            ...
            model = CRFLanguageModel(vocab, d_model=64, d_hidden=32,
                                     n_init_cells=16, max_cells=64,
                                     n_crf_steps=4, k_neighbors=3, cfg=cfg)
            x = torch.randint(0, vocab, (2, 12))
            logits, loss, mets = model(x, targets=x, collect_metrics=True)
            avg_n = (sum(mets.n_cells_per_step) / max(1, len(mets.n_cells_per_step))
                     if mets.n_cells_per_step else 0)
            print(f"routing={use_routing} energy={use_energy} | "
