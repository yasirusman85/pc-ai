...
class TestArithmeticDataset:
    ...
    def test_difficulty_levels(self):
        ...
        easy_ds = ArithmeticDataset(
            n_samples=20, seq_len=32, difficulty="easy", seed=42
        )
        hard_ds = ArithmeticDataset(
            n_samples=20, seq_len=64, difficulty="hard", seed=42
        )
...
class TestDatasetFactory:
    ...
    def test_synthetic_dataset(self):
        ...
        train_ds, val_ds = get_datasets(
            "synthetic", seq_len=32, max_train=100, max_val=20
        )
    ...
    def test_arithmetic_dataset(self):
        ...
        train_ds, val_ds = get_datasets(
            "arithmetic", seq_len=32, max_train=100, max_val=20
        )
...
class TestDataloader:
    ...
    def test_multiple_epochs(self):
        ...
        for epoch in range(3):
            for batch in loader:
                x, y = batch
                assert x.shape[0] <= 4 and x.shape[1] == 32
...
class TestIntegration:
    ...
    def test_full_pipeline(self):
        ...
        # Create dataset
        train_ds, val_ds = get_datasets(
            "synthetic", seq_len=32, max_train=50, max_val=10
        )
